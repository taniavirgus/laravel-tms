...
<div id="profile"
  class="z-50 hidden w-56 overflow-hidden bg-white border divide-y divide-base-200 rounded-xl border-base-200 text-base-700">

  <div class="px-4 py-3 text-sm">
    <span class="block font-semibold text-base-900">{{ Auth::user()->name }}</span>
    <span class="block truncate">{{ Auth::user()->role->label() }}</span>
  </div>
    ...
    @foreach ($menus as $menu)
      <li>
        <a href="{{ $menu->url }}" class="flex items-center gap-2 px-4 py-2 hover:bg-base-100">
          <i data-lucide="{{ $menu->icon }}" class="size-4 text-base-400"></i>
          {{ $menu->name }}
        </a>
      </li>
    @endforeach

...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // Data untuk grafik
        $departmentChart = Department::withCount('employees')
            ->get()
            ->map(fn($dept) => ['label' => $dept->name, 'count' => $dept->employees_count]);

        $positionChart = \App\Models\Position::withCount('employees')
            ->orderByDesc('employees_count')
            ->take(5)
            ->get()
            ->map(fn($pos) => ['label' => $pos->name, 'count' => $pos->employees_count]);
